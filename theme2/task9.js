...
function calcTotalPrice(products) {
    ...
    return products.map(el => el.price).reduce((sum, current) => sum + current);
}
...
function calcTotalPriceByCategory(products, category) {
	// YOUR CODE HERE
    return products.filter(el => el.category === category).map(el => el.price).reduce((sum, current) => sum + current);
}
